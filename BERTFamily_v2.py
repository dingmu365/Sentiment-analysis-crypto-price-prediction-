...
class BERTSentimentClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        outputs = torch.sigmoid(self.output(pooled_output))
        return outputs
...
class DistilBertForSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None):
        ...
        pooled_output = nn.ReLU()(pooled_output)
        pooled_output = self.drop(pooled_output)
        outputs = torch.sigmoid(self.classifier(pooled_output))
...
def load_model(model_type, PATH):
    if model_type == 'bert-base-uncased':
        ...
        model = RobertaSentimentClassifier(n_classes=NUM_CLASSES,pretrained_model_name=model_type,)
...
def get_oof_and_test_preds(model_type, tokenizer,
    ...
    for train_index, val_index in skf.split(x_train, y_train):
        ...
        if model_type == 'bert-base-cased':
            ...
            model = RobertaSentimentClassifier(n_classes=NUM_CLASSES,model_name=model_type)
            model = model.to(device)
        elif model_type == 'roberta-large':
            model = RobertaSentimentClassifier(n_classes=NUM_CLASSES,model_name=model_type)
            model = model.to(device)
...
# roberta_history, roberta_test_outputs = fn.get_oof_and_test_preds(model_type='roberta-large',
